...
(function() {
  ...
  function AppController(scope){
    this.clientId = null;
    this.endpoint = 'aw2gh3lfwaree.iot.us-west-2.amazonaws.com';
    this.accessKey = null;
    this.secretKey = null;
    this.identityPoolId = 'us-west-2:d709951b-880a-4727-a676-cee789e345f6';
  ...
  AppController.prototype.createClient = function() {
    ...
    // get anonymous user credentials
    AWS.config.region = options.regionName;
    var credentials = new AWS.CognitoIdentityCredentials({ IdentityPoolId: identityPoolId });
    /**/console.log("1) Cognito identity credentials object: %O", credentials);
    ...
    credentials.get(function (err) {
      ...
      console.log("credentials have been retrieved");
      /**/console.log("2) Cognito identity credentials object: %O", credentials);

      options.clientId = credentials.identityId.replace('.', '');
      options.accessKey = credentials.accessKeyId;
      options.secretKey = credentials.secretAccessKey;
      options.sessionToken = credentials.sessionToken;
      /**/console.log("options are %O", options);

      // continue with normal code
      var client = self.clients.getClient(options);
      if (!client.connected) {
  ...
  function ClientController(client, logs) {
    this.client = client;
    this.topicName = 'virtual_nile/pdx/bethany';
    this.message = null;
    this.msgs = [];
    this.logs = logs;
    var self = this;
  ...
  */
  ...
  MQTTClient.prototype.computeUrl = function(){
    ...
    var signedQueryString = canonicalQuerystring + '&X-Amz-Signature=' + signature;
    if (sessionToken) {
      signedQueryString = signedQueryString + '&X-Amz-Security-Token=' + encodeURIComponent(sessionToken);
    }

    var requestUrl = 'wss://' + host + canonicalUri + '?' + signedQueryString;
    return requestUrl;
  };
